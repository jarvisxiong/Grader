...
public class main {
	...
	public static void main(String[] args) {
		...
		do {
			...
			for (int i = 0; i < foods.length; i++)
			{
				...
				foods[i].setName(key.nextLine());
				System.out.println("Maximum consumed per gerbil: ");
				foods[i].setAmount(Integer.parseInt(key.nextLine()));
			}
			...
			while (!function.equals("quit") && !function.equals("restart"))
			{
				...
				else if (function.equals("search"))
				{
					...
					System.out.println(searchForGerbil(key.nextLine()));
					...
					{
						searchResult += " " + foods[k].getName() + " - " + result.getFoodNumbers()[k]
								+ "/" + foods[k].getAmount();
					}
	...
	public static String averageFood()
	{
		...
		for (int i = 0; i < allGerbils.length; i++)
		{
			...
			double unroundedAverage = ((double)gerbilSum / (double)totalSum) * 100;
			int average = (int) Math.round(unroundedAverage);
	...
	public static String searchForGerbil (String idName)
	{	
