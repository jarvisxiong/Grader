...
public class AMainProjectStepper extends AClearanceManager implements
		BasicProjectStepper {
...
//	boolean firstTime;
	...
	// Window[] oldWindows;
//	GradedProjectNavigator overviewProjectStepper;
//	GradedProjectOverview overviewProjectStepper;
...
//	Icon studentPhoto;
	...
	// }
	public AMainProjectStepper() {
		overviewProjectStepper = new AnOverviewProjectStepper();
//		overviewProjectStepper = new AGradedProjectNavigator();
//		overviewProjectStepper = new AGradedProjectOverview();
...
//		String oldOnyen = overviewProjectStepper.getOnyen();
//		
...
//		runExecuted = false;
		...
		overviewProjectStepper.setName(project.getStudentAssignment().getStudentName());

//		overviewProjectStepper.setProject(newVal);
...
//			setInternalScore(getGrade());
		double savedScore = featureGradeRecorder.getGrade(overviewProjectStepper.getName(), overviewProjectStepper.getOnyen());
		
		overviewProjectStepper.setInternalScore(savedScore);
		if (!overviewProjectStepper.shouldVisit()) {
			return false;
		}
...
//	}
	...
	public boolean preRunProjectsInteractively() {
		return projectDatabase.getOnyenNavigationList() != null && overviewProjectStepper.getCurrentOnyenIndex() < projectDatabase.getOnyenNavigationList().size();
	}
...
//			hasMoreSteps = false;
		...
		if (aGoToOnyen.isEmpty()) {
			...
			anOnyen= onyens.get(overviewProjectStepper.getCurrentOnyenIndex());
		} else {
...
//		setObjectAdapters();
		...
		if (!retVal) {
			 next();
			 if (!overviewProjectStepper.hasMoreSteps()) {
//				 currentOnyenIndex = filteredOnyenIndex;
				 overviewProjectStepper.setCurrentOnyenIndex(overviewProjectStepper.getFilteredOnyenIndex());
				 return false;
			 }
		}
		overviewProjectStepper.setFilteredOnyenIndex(overviewProjectStepper.getCurrentOnyenIndex());
...
//	}
//	
//	public void setGradedProjectOverview(GradedProjectOverview overviewProjectStepper) {
//		this.overviewProjectStepper = overviewProjectStepper;
//	}
