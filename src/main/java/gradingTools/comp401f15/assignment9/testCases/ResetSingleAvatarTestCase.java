 ...
 */
public class ResetSingleAvatarTestCase extends BasicTestCase {
    ...
    public ResetSingleAvatarTestCase(String avatar) {
        super("Reset " + avatar + " test case");
        avatarName = avatar;
        char inputCharTemp = ' ';
    ...
    private boolean[] checkMovement(Constructor<?> bridgeSceneControllerConstructor, Constructor<?> scenePainterConstructor, Constructor<?> bridgeSceneConstructor, Method getAvatar, Method[] getX, Method[] getY, boolean controllerIsListener, boolean controllerTakesComponent) {
        ...
        if (controllerIsListener) {
            ...
            methods = new MethodExecutionTestCase.MethodEnvironment[]{
                ...
                MethodExecutionTestCase.MethodEnvironment.get(mousePressed, buildMouseEvent(mouseX, mouseY, MouseEvent.MOUSE_PRESSED, component)),      // 3
                MethodExecutionTestCase.MethodEnvironment.get(mouseReleased, buildMouseEvent(mouseX, mouseY, MouseEvent.MOUSE_RELEASED, component)),    // 4
                MethodExecutionTestCase.MethodEnvironment.get(mouseClicked, buildMouseEvent(mouseX, mouseY, MouseEvent.MOUSE_CLICKED, component)),      // 5
                ...
                MethodExecutionTestCase.MethodEnvironment.get(keyPressed, buildKeyEvent(resetChar, resetCode, KeyEvent.KEY_PRESSED, component)),        // 11
                MethodExecutionTestCase.MethodEnvironment.get(keyTyped, buildKeyEvent(resetChar, resetCode, KeyEvent.KEY_TYPED, component)),            // 12
                MethodExecutionTestCase.MethodEnvironment.get(keyReleased, buildKeyEvent(resetChar, resetCode, KeyEvent.KEY_RELEASED, component)),      // 13
                MethodExecutionTestCase.MethodEnvironment.get(MethodExecutionTestCase.CYCLIC_GET_PROPERTY, MethodExecutionTestCase.M0_RET, getX),       // 14
                MethodExecutionTestCase.MethodEnvironment.get(MethodExecutionTestCase.CYCLIC_GET_PROPERTY, MethodExecutionTestCase.M0_RET, getY)        // 15
            };
            ...
            methods = new MethodExecutionTestCase.MethodEnvironment[]{
                ...
                MethodExecutionTestCase.MethodEnvironment.get(MethodExecutionTestCase.CYCLIC_GET_PROPERTY, MethodExecutionTestCase.M0_RET, getX),               // 1
                MethodExecutionTestCase.MethodEnvironment.get(MethodExecutionTestCase.CYCLIC_GET_PROPERTY, MethodExecutionTestCase.M0_RET, getY),               // 2
                MethodExecutionTestCase.MethodEnvironment.get(component, dispatchEvent, buildMouseEvent(mouseX, mouseY, MouseEvent.MOUSE_PRESSED, component)),  // 3
                MethodExecutionTestCase.MethodEnvironment.get(component, dispatchEvent, buildMouseEvent(mouseX, mouseY, MouseEvent.MOUSE_RELEASED, component)), // 4
                MethodExecutionTestCase.MethodEnvironment.get(component, dispatchEvent, buildMouseEvent(mouseX, mouseY, MouseEvent.MOUSE_CLICKED, component)),  // 5
                ...
                MethodExecutionTestCase.MethodEnvironment.get(MethodExecutionTestCase.CYCLIC_GET_PROPERTY, MethodExecutionTestCase.M0_RET, getX),               // 9
                MethodExecutionTestCase.MethodEnvironment.get(MethodExecutionTestCase.CYCLIC_GET_PROPERTY, MethodExecutionTestCase.M0_RET, getY),               // 10
                MethodExecutionTestCase.MethodEnvironment.get(component, dispatchEvent, buildKeyEvent(resetChar, resetCode, KeyEvent.KEY_PRESSED, component)),  // 11
                MethodExecutionTestCase.MethodEnvironment.get(component, dispatchEvent, buildKeyEvent(resetChar, resetCode, KeyEvent.KEY_TYPED, component)),    // 12
                MethodExecutionTestCase.MethodEnvironment.get(component, dispatchEvent, buildKeyEvent(resetChar, resetCode, KeyEvent.KEY_RELEASED, component)), // 13
                MethodExecutionTestCase.MethodEnvironment.get(MethodExecutionTestCase.CYCLIC_GET_PROPERTY, MethodExecutionTestCase.M0_RET, getX),               // 14
                MethodExecutionTestCase.MethodEnvironment.get(MethodExecutionTestCase.CYCLIC_GET_PROPERTY, MethodExecutionTestCase.M0_RET, getY)                // 15
            };
        } else {
        ...
        retVal[0] = ExecutionResultTools.checkNotException(exData, 3)
                ...
                && ExecutionResultTools.checkNotException(exData, 6)
                && ExecutionResultTools.checkNotException(exData, 7)
                && ExecutionResultTools.checkNotException(exData, 8);
        
        retVal[1] = ExecutionResultTools.checkNEqual(exData, 1, 9)
                || ExecutionResultTools.checkNEqual(exData, 2, 10);
        
        retVal[2] = ExecutionResultTools.checkNotException(exData, 11)
                && ExecutionResultTools.checkNotException(exData, 12)
                && ExecutionResultTools.checkNotException(exData, 13);
        
        retVal[3] = ExecutionResultTools.checkNEqual(exData, 9, 14)
                || ExecutionResultTools.checkNEqual(exData, 10, 15);
        
        retVal[4] = ExecutionResultTools.checkEqual(exData, 1, 14)
                && ExecutionResultTools.checkEqual(exData, 2, 15);
    ...
    private static String buildMessage(boolean[] notes) {
        ...
        if (notes[1] == false) {
            ret.append("Avatar did not move from start (so can't autograde)\n");
        }
        if (notes[2] == false) {
        ...
        if (notes[4] == false) {
            ret.append("Avatar does not move to starting location\n");
        }
        return ret.toString();
    }
}
