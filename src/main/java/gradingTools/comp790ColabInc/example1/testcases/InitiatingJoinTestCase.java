...
public class InitiatingJoinTestCase extends BasicTestCase {
	IMInputGenerator imInputGenerator;
	public InitiatingJoinTestCase(IMInputGenerator anOutputBasedInputGenerator) {
...
//			IMInputGenerator anOutputBasedInputGenerator = new AnIMInputGenerator();
			...
			if (imInputGenerator.allJoined())
				return pass();
			double credit = (imInputGenerator.getNumClientsJoined()/3.0);
			if (credit == 0)
				return fail("No clients joined session");
			return partialPass(credit, imInputGenerator.getNumClientsJoined() + " clients joined rather than " + 3);
