...
public class rolledOneOnlyOnceTest extends BasicTestCase {

	public rolledOneOnlyOnceTest() {
		super("rolled one only once test case");
	...
	public TestCaseResult test(Project project, boolean autoGrade)
		...
		for (ClassDescription description : manager.getClassDescriptions() ) {
			...
			for (Method method : javaClass.getDeclaredMethods()) {
				boolean correctName = method.getName().toLowerCase().equals("rolledoneonlyonce");//decided to ignore case here
    			...
    			if(correctName&&correctVisibility&&correctReturnType)
					...
					boolean b1=result1.equals(Boolean.FALSE);
					boolean b2=result2.equals(Boolean.FALSE);
					boolean b3=result3.equals(Boolean.TRUE);
					if(b1&&b2&&b3)return pass();
						...
						if(!b1){
							numwrong++;
							message+="does not return false when dice values are 1 and 1\n";
						}
						if(!b2){
							numwrong++;
						...
						if(!b3){
							numwrong++;
							message+="does not return true when dice values are 1 and 3\n";
						}
		...
		}return fail("No method rolledOneOnlyOnce found in program, grader please confirm.");
	}
