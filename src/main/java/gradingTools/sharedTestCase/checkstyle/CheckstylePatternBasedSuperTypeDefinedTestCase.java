...
public class CheckstylePatternBasedSuperTypeDefinedTestCase 
	extends CheckStyleCountingWarningsTestCase {
	public static final String WARNING_NAME = "expectedSuperTypes";

//	protected String descriptor;
	 String expectedSuperType;
	 public CheckstylePatternBasedSuperTypeDefinedTestCase(
			 String aSuperType, double aPenaltyPerMistake ) {
	        super(null, "Subtypes of " + aSuperType + " defined", aPenaltyPerMistake );
	        expectedSuperType = aSuperType;
	  }
	 @Override
	...
	public String regexLineFilter() {
		return "(.*)"+ WARNING_NAME + "(.*)missing supertype:(.*)" + expectedSuperType;

				
//				MethodExecutionTest.toRegex(
//				
//				"In type(.*)" + getActualType() +"(.*)missing supertype:(.*)" + expectedSuperType);
//				
	}
