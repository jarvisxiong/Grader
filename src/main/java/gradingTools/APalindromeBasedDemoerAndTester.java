 ...
 */
public class APalindromeBasedDemoerAndTester implements  GraderDemoerAndTester{
	 String[] args ;
	public final  String TEST_DIR = "Test Data/Test 110 F13 Assignments";
	public final  String CORRECT_DIR = "Test Data/Correct 110 F13 Results";
	
	 String directory;
	
	 boolean  generatingCorrectDir = false;
	
	 boolean autoProceed = false;
//	 boolean  generatingCorrectDir = true;

	
	

	 DemoAndTestingClearanceManager clearanceManager = new ADemoAndTestingClearanceManager();
//	 boolean autoProceed;
//	 long autoProceedPauseTime = 4000;
	 
	public APalindromeBasedDemoerAndTester(String[] anArgs) {
		args = anArgs;
	}
	/* (non-Javadoc)
...
//		ObjectEditor.setDefaultAttribute(AttributeNames.SHOW_SYSTEM_MENUS, false);
		GraderDemoerAndTester demoerAndTester = new APalindromeBasedDemoerAndTester(anArgs);
...
//	}
	 ...
	 void waitForUserOrLongSleep() {
		if (clearanceManager.isAutoProceed())
			ThreadSupport.sleep(clearanceManager.getAutoPauseTime()*1000*30);
		else
			clearanceManager.waitForClearance();
	}
	
	 void waitForUserOrMediumSleep() {
		if (clearanceManager.isAutoProceed())
			ThreadSupport.sleep(clearanceManager.getAutoPauseTime()*1000*5);
		else
			clearanceManager.waitForClearance();
	}
